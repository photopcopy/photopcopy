	...
	constructor(initialSettings: Partial<UserSettings>) {
		...
		for (const prop in this.current) {
			if (prop in initialSettings) {
				this.set(
					prop as keyof UserSettings,
					initialSettings[prop as keyof UserSettings] as UserSettings[keyof UserSettings],
					false,
				);
			} else {
				this.set(
					prop as keyof UserSettings,
					this.current[prop as keyof UserSettings] as UserSettings[keyof UserSettings],
					false,
				);
			}
		}
